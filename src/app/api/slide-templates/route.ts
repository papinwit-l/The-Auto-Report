...
import type { SlideTemplateFile } from "@/types";
...
async function readTemplates(): Promise<SlideTemplateFile[]> {
  try {
...
async function writeTemplates(templates: SlideTemplateFile[]): Promise<void> {
...
export async function POST(request: NextRequest) {
  try {
    const body: SlideTemplateFile = await request.json();
    const templates = await readTemplates();
